...
import { getSoftwareList } from '@/api/softwareStore/software.ts'
 ...
 * app 列表
 */
const request_getSoftwareList = async () => {
  try {
...
<!--        <div class="nhd"></div>-->
    ...
    <main class="store_body" mt flex>
      <div class="l"
           w-70 h-full mr-11 b-r-1 b-r-solid b-r-gray:10 pr-4>
        <div class="top">
      ...
      <div class="r" w-full="">
        <header class="top" flex items-center>
          ...
          <el-input>
            <template #append>
              <el-button text size="small" :icon="Search"></el-button>
            </template>
          </el-input>
        </header>
        <div class="category"
          ...
          <div class="app_cat_title">{{ item[0] }}</div>
          <main class="store_list" flex
                gap-6
                flex-wrap
                bg-gray:10
                mt-1
                ...
                rounded="sm"
          >
            <div class="store_item_wrap"
                 :key="idx"
                 v-for="(it, idx) in item[1]">
              <div class="top"
                   flex
                   items-center
                   flex-col
              >
                <div
                  ...
                  class="app_icon"></div>
                <el-popover
                  popper-class="app_popover_class"
                >
                  <template #reference>
                    <div class="app_name"
                         text-center
                ...
                <el-icon
                  transition-colors
                  hover:text-white:50 mt-1 cursor-pointer><Download /></el-icon>
              </div>
...
.store_text {
  ...
  color: transparent;
  -webkit-text-stroke-color: #74f574;
  animation: ni-hong-deng .2s linear infinite;
  width: fit-content;
}
...
.beforeAndAfter {
  ...
  left: 0;
  z-index: 0;
  animation: top_to_botm .5s linear infinite;
}
.nhd {
  position: relative;
  width: 5px;
